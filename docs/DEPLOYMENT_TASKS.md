...
|----|-------|------|---------|--------|----------------|
| D01 | 1 — Domain & DNS | Purchase `.in` domain | Namecheap | ☐ | Prerequisite for T03 |
| D02 | 1 | Add domain to Cloudflare | Cloudflare | ☐ | See `docs/cloudflare-dns.md` |
| D03 | 1 | Add wildcard A record (`*` → Oracle VM IP) | Cloudflare DNS | 🟡 | T03 — config documented |
| D04 | 1 | Enable wildcard SSL (Full strict) | Cloudflare SSL | 🟡 | T03 — no Certbot on server |
...
| D11 | 2 | Configure Nginx wildcard `*.shikshaportal.in` | Oracle VM | 🟡 | T03 — `docs/nginx-wildcard.conf` |
...
| D22 | 5 | Setup UptimeRobot monitor | UptimeRobot | ☐ | Alert on `shikshaportal.in/up` |
...
- Cloudflare DNS: [`docs/cloudflare-dns.md`](cloudflare-dns.md)
